"""SQLean Python Driver — Drop-in replacement for standard library `sqlite3`.
Backed by the 100% pure-Mojo database engine compiled into `sqlean.so`.
"""
...
_LIB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sqlean.so")
if not os.path.exists(_LIB_PATH):
    # Try current working directory
    _LIB_PATH = os.path.abspath("sqlean.so")
...
def sqlite_version() -> str:
    """Returns the SQLean SQLite library version."""
    if _lib:
...
class Connection:
    def __init__(self, database: str):
        if _lib is None:
            raise OperationalError("Could not load native sqlean.so library.")

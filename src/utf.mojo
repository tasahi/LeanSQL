...
def write_utf8[origin: Origin[mut=True]](p: Pointer[UInt8, origin], c: UInt32) -> Int:
    """ Encodes a Unicode codepoint `c` into 1 to 4 UTF-8 bytes at `p`.
    Returns the number of bytes written.
    """
    if c < 0x80:
        p[unsafe_offset=0] = UInt8(c & 0xFF)
        return 1
    elif c < 0x800:
        p[unsafe_offset=0] = UInt8(0xC0 | ((c >> 6) & 0x1F))
        p[unsafe_offset=1] = UInt8(0x80 | (c & 0x3F))
        return 2
    elif c < 0x10000:
        p[unsafe_offset=0] = UInt8(0xE0 | ((c >> 12) & 0x0F))
        p[unsafe_offset=1] = UInt8(0x80 | ((c >> 6) & 0x3F))
        p[unsafe_offset=2] = UInt8(0x80 | (c & 0x3F))
        return 3
    else:
        p[unsafe_offset=0] = UInt8(0xF0 | ((c >> 18) & 0x07))
        p[unsafe_offset=1] = UInt8(0x80 | ((c >> 12) & 0x3F))
        p[unsafe_offset=2] = UInt8(0x80 | ((c >> 6) & 0x3F))
        p[unsafe_offset=3] = UInt8(0x80 | (c & 0x3F))
        return 4


def read_utf8[origin: Origin](p: Pointer[UInt8, origin], mut cp: UInt32) -> Int:
    """ Decodes a single Unicode codepoint from UTF-8 byte stream `p` into `cp`.
    Returns the number of bytes consumed (1..4).
    """
    var b0 = UInt32(p[unsafe_offset=0])
    if b0 < 0x80:
        cp = b0
        return 1
    elif (b0 & 0xE0) == 0xC0:
        var b1 = UInt32(p[unsafe_offset=1])
        cp = ((b0 & 0x1F) << 6) | (b1 & 0x3F)
        return 2
    elif (b0 & 0xF0) == 0xE0:
        var b1 = UInt32(p[unsafe_offset=1])
        var b2 = UInt32(p[unsafe_offset=2])
        cp = ((b0 & 0x0F) << 12) | ((b1 & 0x3F) << 6) | (b2 & 0x3F)
        return 3
    elif (b0 & 0xF8) == 0xF0:
        var b1 = UInt32(p[unsafe_offset=1])
        var b2 = UInt32(p[unsafe_offset=2])
        var b3 = UInt32(p[unsafe_offset=3])
...
def utf8_char_length[origin: Origin](p: Pointer[UInt8, origin], num_bytes: Int) -> Int:
    """ Returns the number of logical UTF-8 characters across `num_bytes`."""
    var count: Int = 0
    var offset: Int = 0
    while offset < num_bytes:
        var b = p[unsafe_offset=offset]

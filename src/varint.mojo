...
def put_varint64(p: UnsafePointer[UInt8, MutAnyOrigin], val: UInt64) -> Int:
...
def put_varint(p: UnsafePointer[UInt8, MutAnyOrigin], v: UInt64) -> Int:
    ...
    if v <= 0x7F:
        ...
        return 1
    if v <= 0x3FFF:
        p[0] = UInt8(((v >> 7) & 0x7F) | 0x80)
        p[1] = UInt8(v & 0x7F)
        return 2
    return put_varint64(p, v)

def get_varint(p: UnsafePointer[UInt8, ImmutAnyOrigin], mut v: UInt64) -> Int:
    ...
    """
    var i_key: UInt64 = UInt64(p[0])
    if i_key < 0x80:
        v = i_key
        return 1

    var offset: Int = 1
    var x: UInt64 = UInt64(p[offset])
    i_key = (i_key << 7) ^ x
    if x < 0x80:
        i_key ^= UInt64(0x4000)
        v = i_key
...
def get_varint_value(p: UnsafePointer[UInt8, ImmutAnyOrigin]) -> Int64:
...
def get_varint32(p: UnsafePointer[UInt8, ImmutAnyOrigin], mut v: UInt32) -> Int:
...
def put_varint32(p: UnsafePointer[UInt8, MutAnyOrigin], v: UInt32) -> Int:
...
def decode_varint(bytes: List[UInt8]) -> Tuple[UInt64, Int]:
    """ Convenience helper: decodes a byte list into (value, bytes_read)."""
    var p = alloc[UInt8](len(bytes))
    for i in range(len(bytes)):
        p[i] = bytes[i]
    var v: UInt64 = 0
    var immut_p = UnsafePointer[UInt8, ImmutAnyOrigin](other=p)
    var n = get_varint(immut_p, v)
    p.free()
    return Tuple(v, n)

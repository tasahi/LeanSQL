""" Python C Extension Module for LeanSQL.

Allows LeanSQL to be compiled into a native `.so` Python extension module
exporting `PyInit_leansql` and callable from standard Python scripts via `import leansql`.
"""
...
from src.core.types import MutAnyOrigin
...
def leansql_open(path_obj: PythonObject) raises -> PythonObject:
...
def leansql_execute(target_obj: PythonObject, sql_obj: PythonObject) raises -> PythonObject:
...
def leansql_fetch_all(target_obj: PythonObject, sql_obj: PythonObject) raises -> PythonObject:
...
def leansql_commit(target_obj: PythonObject) raises -> PythonObject:
...
def PyInit_leansql() abi("C") -> PythonObject:
    ...
    try:
        ...
        mb.def_function[leansql_open]("open", "Opens a connection and returns handle")

# SQLean: Programmer's Reference Manual

This technical manual details the internal architecture, subsystem decomposition, type systems, and C/Python extension APIs for developers working on or extending the **100% Pure-Mojo SQLean database engine**.

---

## 1. Engine Subsystems Overview

The SQLean architecture faithfully implements the layered SQLite database engine without any external C runtime dependencies:
...
### A. OS Interface & Storage (`src/vfs.mojo`, `src/pager.mojo`, `src/journal.mojo`)
...
### B. B-Tree & Binary Codecs (`src/btree.mojo`, `src/btree_cell.mojo`, `src/record.mojo`, `src/serial.mojo`, `src/page_header.mojo`)
...
### C. Bytecode Engine (`src/vdbe.mojo`, `src/opcode.mojo`)
...
### D. Tokenizer, Parser & Query Planner (`src/tokenizer.mojo`, `src/parser.mojo`, `src/uast.mojo`, `src/optimizer.mojo`)
...
### E. Connection & Evaluation (`src/connection.mojo`, `src/cursor.mojo`, `src/schema.mojo`, `src/functions.mojo`)
...
### A. Full-Text Search & BM25 (`src/fts.mojo`)
...
### B. SIMD Vector Similarity (`src/vector.mojo`)
...
### C. Cryptography & Encodings (`src/crypto.mojo`)
...
### A. C-ABI Header (`include/sqlean.h` & `src/c_api.mojo`)
...
### B. Python C-Extension (`src/py_export.mojo`)
Compiled with:
```bash
mojo build --emit shared-lib -I . src/py_export.mojo -o sqlean.so
```
Exports `PyInit_sqlean` with `open`, `execute`, `fetch_all`, `commit`, `close`, `version`.
...
1. Implement the kernel in Mojo (e.g., `src/functions.mojo` or dedicated module).
2. Register the function name in `evaluate_scalar_func` in `src/functions.mojo`:
```mojo

"""Comprehensive Verification of the LeanSQL Python Drop-in Driver (leansql_driver.py).
Tests DB-API 2.0 interface compatibility over compiled native leansql.so.
"""
...
import leansql_driver as leansql


class TestLeanSQLPythonDriver(unittest.TestCase):
    def setUp(self):
        self.db_path = "test_driver_suite.db"
        if os.path.exists(self.db_path):
            os.remove(self.db_path)
        self.con = leansql.connect(self.db_path)
    ...
    def test_version_info(self):
        ver = leansql.sqlite_version()
        self.assertIn("LeanSQL", ver)
